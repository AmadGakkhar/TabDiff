#!/usr/bin/env bash
# Approach 1 — Balanced-Bagging Ensemble (M=4) for fraud_oracle (s123 split)
#
# Stage 1  prepare_foracle_app1.py — build 4 balanced partitions, register
...
LOGDIR=logs/foracle_app1
OUTDIR=synthetic_outputs/foracle_app1
ORIG_CFG=tabdiff/configs/tabdiff_configs.toml
MEMBER_CFG=tabdiff/configs/tabdiff_configs_foracle_app1.toml
...
python -u prepare_foracle_app1.py 2>&1 | tee -a "${LOGDIR}/prepare.log"
...
for m in $(seq 1 $M); do
    ...
    python -u main.py \
        --dataname "$NAME" --mode train --no_wandb --exp_name "$NAME" --resume \
        >> "$TRAIN_LOG" 2>&1
    rc=$?
    cp "$BAK_CFG" "$ORIG_CFG"
    if [[ $rc -ne 0 ]]; then
...
if not files:
    print("ERROR: no member fraud files found", file=sys.stderr); sys.exit(1)
...
log "=== Approach 1 (fraud_oracle) run finished ==="
