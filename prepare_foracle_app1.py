"""Build M=4 balanced partitions of the fraud_oracle dataset for Approach 1
(balanced-bagging ensemble of TabDiff generators).

Same logic as prepare_expv01_app1.py — identical 33-col fraud_oracle schema —
but sourced from the true_baseline_s123 train split and registered under
foracle_m* names so it does not clobber the exp-v01 members.
...
Run once before run_foracle_app1.sh.
"""
import os
import json
import shutil
...
SRC        = "data/fraud_oracle/true_baseline_s123_train.csv"
DROP_COLS  = ["PolicyNumber"]   # row identifier, not a feature
TARGET     = "FraudFound_P"
M          = 4
INFO_DIR   = "data/Info"
CONFIG_OUT = "tabdiff/configs/tabdiff_configs_foracle_app1.toml"

# Schema after dropping PolicyNumber — identical to the exp-v01 / 'fraud' dataset.
N_COLS         = 32
...
def main():
    ...
    n_maj    = len(fraud)   # strict 1:1 balance
    ...
    tmp_dir = "data/foracle_app1_partitions"
    ...
    print("\nPreparation complete. Run run_foracle_app1.sh to train and generate.")
